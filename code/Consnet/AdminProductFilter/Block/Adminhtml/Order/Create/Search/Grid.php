 ...
 */
namespace Consnet\AdminProductFilter\Block\Adminhtml\Order\Create\Search;
...
class Grid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    protected function _prepareCollection()
    {
        ...
        $sharedcatalog = $om->create('Magento\SharedCatalog\Model\ProductItemManagement');
        
        $skus = array();
        
        $searchCriteriaBuilder = $om->create('\Magento\Framework\Api\SearchCriteriaBuilder');
        $sharedCatalogProductItemRepository = $om->create('\Magento\SharedCatalog\Api\ProductItemRepositoryInterface');
        $searchCriteriaBuilder->addFilter(
            \Magento\SharedCatalog\Api\Data\ProductItemInterface::CUSTOMER_GROUP_ID,
            $group_id,
            \Magento\SharedCatalog\Model\ProductItemManagement::EQUAL_VALUE
        );
        $searchCriteria = $searchCriteriaBuilder->create();
        try {
            $searchResults = $sharedCatalogProductItemRepository->getList($searchCriteria);
        } catch (\InvalidArgumentException $e) {
            throw new LocalizedException(__('Cannot load product items for shared catalog'));
        }
        $groupitems = $searchResults->getItems();
        
        foreach ($groupitems as $item) {
            array_push($skus, $item->getSku());
        }
        
        $newcollection = $collection->addAttributeToFilter("sku", array('in' => $skus));
        
        $this->setCollection($newcollection);//$collection);
